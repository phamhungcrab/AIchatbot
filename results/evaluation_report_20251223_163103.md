...
> **Thời gian đánh giá:** 2025-12-23 16:31:03  
...
- **Công thức Bayes:** `P(topic|X) ∝ P(topic) × ∏ P(word_i|topic)`
- **Smoothing:** Laplace Smoothing (alpha=0.1)

### Công thức tính Confidence

**Bước 1: Tính Log Probability cho mỗi topic**
```
log P(topic|X) = log P(topic) + Σ log P(word_i|topic)
```

**Bước 2: Chuyển sang xác suất thực bằng Softmax**
```
P(topic_c|X) = exp(log P(topic_c|X)) / Σ exp(log P(topic_k|X))
```

**Bước 3: Confidence = max probability**
```
Confidence = max(P(topic_1|X), P(topic_2|X), ..., P(topic_n|X))
```

> 📌 **Ví dụ:** Nếu có 3 topics với xác suất [0.65, 0.25, 0.10] → Confidence = **0.65** (65%)
